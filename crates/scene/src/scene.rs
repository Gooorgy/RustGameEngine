...
impl SceneManager {
    ...
    pub fn init_scene(&mut self, engine_context: &mut EngineContext) {
        let _ = self.scene_components.iter_mut().map(
            |component_registration: &mut ComponentRegistration| {
                component_registration
                    .component
                    .borrow_mut()
                    .setup(engine_context);
            },
        );
    }
